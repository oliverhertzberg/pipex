...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/06 11:07:31 by ohertzbe          #+#    #+#             */
/*   Updated: 2024/03/07 08:22:56 by ohertzbe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libft.h"

int	static	count_words3(char const *s, char c)
{
	int		i;
	int		count;
	char	flag;
...
static void	assemble(char **strings, char const *s, char c, int word_count)
{
	...
	while (x.j < word_count)
	{
		...
		while (x.f && (s[x.i] != x.f || (s[x.i] == x.f && s[x.i - 1] == '\\')))
			x.i++;
		x.end = x.i;
		strings[x.j++] = ft_substr3(s, x.start, (x.end - x.start), &x.f);
